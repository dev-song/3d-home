...
function Portfolio({ lang, portfolioInfo }) {
  return portfolioInfo.map((eachInfo, index) => {
    const {
      title,
      subtitle,
      description,
    ...
    return (
      <div key={index} className='portfolio-item'>
        <h4 className='portfolio-item__title'>{lang === 'ko' ? titleKR : title}</h4>
        <h5
        ...
        <div className='portfolio-item__links'>
          <a
            className='portfolio-item__link link-project'
            href={url}
            target='_blank'
          ...
          <a
            className='portfolio-item__link link-github'
            href={github}
            target='_blank'
            rel='noopener noreferrer'
          >
